...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_checkpoint:
            attn_output = checkpoint(self.attention, self.ln_1(x))
            x = x + attn_output
            if self.mlp_ratio > 0:
...
class TiTokEncoder(nn.Module):
    def __init__(self, image_size, patch_size, model_size, num_latent_tokens, token_size, use_checkpoint=True):
        super().__init__()
        ...
        for i in range(self.num_layers):
            self.transformer.append(ResidualAttentionBlock(
                self.width, self.num_heads, mlp_ratio=4.0, use_checkpoint=use_checkpoint
            ))
...
class TiTokDecoder(nn.Module):
    ...
    def forward(self, z_quantized):
        ...
        mask_tokens = torch.cat([_expand_token(self.class_embedding, mask_tokens.shape[0]).to(mask_tokens.dtype),
                                    mask_tokens], dim=1)
        mask_tokens = mask_tokens + self.positional_embedding[None, :mask_tokens.shape[1], :].to(mask_tokens.dtype)
