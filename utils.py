...
def get_experiment_dir(root_dir, args):
    ...
    if args.fixed_spatial:
        root_dir += '-FixedSpa'
    if hasattr(args, 'enable_xformers_memory_efficient_attention') and args.enable_xformers_memory_efficient_attention:
        root_dir += '-Xfor'
...
#         # match_name = name.removeprefix('module.').removeprefix('_orig_mod.')
#         match_name = name
#         ema_params[match_name].mul_(decay).add_(param.data.to(ema_params[match_name].device), alpha=1 - decay)
...
# function that cast type of a value to the type of a reference value
def cast_type(value, ref_value):
...
def parse_args():
    ...
    known_keys = vars(args).keys()

    for arg in unknown:
        if arg.startswith(("-", "--")):
            parser.add_argument(arg.split('=')[0])
            print('Adding new arg from command line:', arg.split('=')[0])
    ...
    for k, v in vars(args).items():
        ...
        if k in known_keys:
            ...
            print(f'Warning: {k} is not in the known keys of args!')
            if '.' in k:
                keys = k.split('.')
                sub_config = configs
                for key in keys[:-1]:
                ...
                sub_config[keys[-1]] = cast_type(v, sub_config.get(keys[-1]))
                
            else:
                configs[k] = cast_type(v, configs.get(k))
