...
def get_experiment_dir(root_dir, args):
    ...
    if hasattr(args, 'fixed_spatial') and args.fixed_spatial:
        root_dir += '-FixedSpa'
    ...
    if hasattr(args, 'gradient_checkpointing') and args.gradient_checkpointing:
        root_dir += '-Gc'
    if hasattr(args, 'mixed_precision') and args.mixed_precision or hasattr(args, 'mixed_precision_16bit') and args.mixed_precision_16bit:
        root_dir += '-Amp'
    if hasattr(args, 'image_size') and args.image_size == 512:
        root_dir += '-512'
    if hasattr(args, 'load_latent') and args.load_latent:
        root_dir += '-loadlatent'
    else:
        root_dir += '-loadpixel'
