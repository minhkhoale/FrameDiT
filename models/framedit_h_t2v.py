...
class MatrixAttentionProcessor:
    """
    Processor for Matrix Attention to be used with diffusers.
    Handles reshaping of hidden states to match MatrixLinear expectations.
    """
    def __init__(self):
        pass
    ...
    def __call__(
        ...
        # Input shape from diffusers: (batch_size, sequence_length, dim)
        # MatrixLinear expects: (..., row_dim, col_dim) where sequence_length = row_dim * col_dim? 
        # OR sequence_length = row_dim (tokens) and dim = col_dim (channels)
        
        B, T, N, D = hidden_states.shape
        
        query = attn.linear_q(hidden_states)
        key = attn.linear_k(hidden_states)
        value = attn.linear_v(hidden_states)

        # Reshape for Multi-Head Attention
        # We need to split both row and col dimensions
        # Current shape: (B, T, N_qk, D)
        
        # 1. Split columns (feature dim) into col_heads
        # 2. Split rows (token dim) into row_heads
        
        B, T, N_qk, D_qk = query.shape
        B, T, N_v, D_v = value.shape
        
        # Check dimensions
        # num_col_heads = attn.num_col_heads
        # num_row_heads = attn.num_row_heads
        
        head_row_dim = attn.head_row_dim
        qk_head_col_dim = attn.qk_head_col_dim
        v_head_col_dim = attn.v_head_col_dim

        # Reshape Q, K, V to: (B, num_row_heads, num_col_heads, head_row_dim, head_col_dim)
        # Note: We group heads into the batch dimension or handle them explicitly
        
        def reshape_heads(x):
            # B, T, (C N), (R D) -> B (C R) T (N D)
            B, T, CN, RD = x.shape
        ...
        if attn.attention_mode == 'xformers':
           raise NotImplementedError("MatrixAttention does not support xformers attention mode.")
        elif attn.attention_mode == 'flash':
            ...
            with torch.backends.cuda.sdp_kernel(enable_flash=True, enable_mem_efficient=True, enable_math=True):
                x = torch.nn.functional.scaled_dot_product_attention(query, key, value) # (B, col_num_head * row_num_head, num_tokens, N*D)
        elif attn.attention_mode == 'math':
            attn_prob = (query @ key.transpose(-2, -1)) * attn.scale  # (B, col_num_head * row_num_head, T, T)
            attn_prob = attn_prob.softmax(dim=-1)
            attn_prob = attn.attn_drop(attn_prob)
            x = (attn_prob @ value) # B, col_num_head * row_num_head, T, N*D
        elif attn.attention_mode == 'flash_v2':
...
class MatrixAttention(nn.Module):
    """
    A Diffusers-compatible wrapper for the MatrixAttention block.
    """
    def __init__(
        self,
        col_dim: int,
        ...
        u_type='param',
        attention_mode='math',
        processor: Optional[MatrixAttentionProcessor] = None,
    ):
        super().__init__()
        ...
        # Q, K, V Projections using MatrixLinear
        self.linear_q = MatrixLinear(
            (self.col_dim, self.row_dim), 
            (self.qk_col_dim, self.row_dim), 
            bias=use_bias, bias_type=bias_type, u_type=u_type
        )
        self.linear_k = MatrixLinear(
            (self.col_dim, self.row_dim), 
            (self.qk_col_dim, self.row_dim), 
            bias=use_bias, bias_type=bias_type, u_type=u_type
        )
        self.linear_v = MatrixLinear(
            (self.col_dim, self.row_dim), 
            (self.v_col_dim, self.row_dim), 
            bias=use_bias, bias_type=bias_type, u_type=u_type
        )
        self.proj_v = MatrixLinear(
            (self.v_col_dim, self.row_dim), 
            (self.col_dim, self.row_dim), 
            bias=use_bias, bias_type=bias_type, u_type=u_type
        )

        self.head_row_dim = self.row_dim // self.num_row_heads
        self.qk_head_col_dim = self.qk_col_dim // self.num_col_heads
        self.v_head_col_dim = self.v_col_dim // self.num_col_heads
        self.attention_mode = attention_mode

        # Set processor
        self.set_processor(processor if processor is not None else MatrixAttentionProcessor())

    def set_processor(self, processor: "MatrixAttentionProcessor"):
...
class BasicTransformerBlock_(nn.Module):
    ...
    def __init__(
        ...
        **attn_kwargs
        ...
        assert self.only_cross_attention is False, "only support self attn"
        ...
        if positional_embeddings == "sinusoidal":
            self.pos_embed = SinusoidalPositionalEmbedding(row_dim, max_seq_length=num_positional_embeddings)
        else:
            self.pos_embed = None
        ...
        if self.use_ada_layer_norm:
            self.norm1 = AdaLayerNorm(row_dim, num_embeds_ada_norm)
        elif self.use_ada_layer_norm_zero:
            self.norm1 = AdaLayerNormZero(row_dim, num_embeds_ada_norm)
        else:
        ...
        self.norm3 = nn.LayerNorm(row_dim, elementwise_affine=norm_elementwise_affine, eps=norm_eps)
        self.ff = FeedForward(row_dim, dropout=dropout, activation_fn=activation_fn, final_dropout=final_dropout)

        # 4. Fuser
        if attention_type == "gated" or attention_type == "gated-text-image":
            self.fuser = GatedSelfAttentionDense(row_dim, cross_attention_dim, num_row_heads, num_row_heads)

        # 5. Scale-shift for PixArt-Alpha.
        if self.use_ada_layer_norm_single:
            self.scale_shift_table = nn.Parameter(torch.randn(6, row_dim) / row_dim**0.5)
    ...
    def forward(
        ...
        attn_output = self.attn1(
            rearrange(norm_hidden_states, '(B N) T D -> B T N D', N=self.col_dim),
...
class FrameDiT_H_T2V(ModelMixin, ConfigMixin):
    ...
    def __init__(
        self,
        col_dim: int,
        num_col_attn_heads: int,
        num_row_attn_heads: int = 16,
        qk_col_attn_head_dim: int = 88,
        v_col_attn_head_dim: int = 88,
        row_attn_head_dim: int = 88,
        ...
        self.num_col_attn_heads = num_col_attn_heads
        self.num_row_attn_heads = num_row_attn_heads
        self.qk_col_attn_head_dim = qk_col_attn_head_dim
        self.v_col_attn_head_dim = v_col_attn_head_dim
        self.row_attn_head_dim = row_attn_head_dim

        qk_col_inner_dim = num_col_attn_heads * qk_col_attn_head_dim
        ...
        if self.is_input_continuous:
            ...
            if use_linear_projection:
                self.proj_in = linear_cls(in_channels, row_inner_dim)
            else:
                self.proj_in = conv_cls(in_channels, row_inner_dim, kernel_size=1, stride=1, padding=0)
            ...
            self.latent_image_embedding = ImagePositionalEmbeddings(
                num_embed=num_vector_embeds, embed_dim=row_inner_dim, height=self.height, width=self.width
            )
        ...
        self.temporal_transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock_(
                    ...
                    qk_col_dim=qk_col_inner_dim,
                    v_col_dim=v_col_inner_dim,
                    num_col_heads=num_col_attn_heads,
                    num_row_heads=num_row_attn_heads,
                    dropout=dropout,
                    activation_fn=activation_fn,
                    num_embeds_ada_norm=num_embeds_ada_norm,
                    attention_bias=attention_bias,
        ...
        if self.is_input_continuous:
            ...
            if use_linear_projection:
                self.proj_out = linear_cls(row_inner_dim, in_channels)
            else:
                self.proj_out = conv_cls(row_inner_dim, in_channels, kernel_size=1, stride=1, padding=0)
        elif self.is_input_vectorized:
            self.norm_out = nn.LayerNorm(row_inner_dim)
            self.out = nn.Linear(row_inner_dim, self.num_vector_embeds - 1)
        elif self.is_input_patches and norm_type != "ada_norm_single":
            self.norm_out = nn.LayerNorm(row_inner_dim, elementwise_affine=False, eps=1e-6)
            self.proj_out_1 = nn.Linear(row_inner_dim, 2 * row_inner_dim)
            self.proj_out_2 = nn.Linear(row_inner_dim, patch_size * patch_size * self.out_channels)
        elif self.is_input_patches and norm_type == "ada_norm_single":
            self.norm_out = nn.LayerNorm(row_inner_dim, elementwise_affine=False, eps=1e-6)
            self.scale_shift_table = nn.Parameter(torch.randn(2, row_inner_dim) / row_inner_dim**0.5)
            self.proj_out = nn.Linear(row_inner_dim, patch_size * patch_size * self.out_channels)
        ...
        if norm_type == "ada_norm_single":
            ...
            self.adaln_single = AdaLayerNormSingle(row_inner_dim, use_additional_conditions=self.use_additional_conditions)

        self.caption_projection = None
        if caption_channels is not None:
            self.caption_projection = PixArtAlphaTextProjection(in_features=caption_channels, hidden_size=row_inner_dim)

        self.gradient_checkpointing = False
        self.n_gradient_checkpointing_blocks = 999

        # define temporal positional embedding
        temp_pos_embed = self.get_1d_sincos_temp_embed(row_inner_dim, video_length) # 1152 hidden size
    ...
    def _set_gradient_checkpointing(self, module, value, n_blocks=999):
        self.gradient_checkpointing = value
        self.n_gradient_checkpointing_blocks = n_blocks
    ...
    def enable_gradient_checkpointing(self, n_first_temporal_block=999) -> None:
        ...
        *checkpoint activations* in other frameworks).
        ...
        self.apply(partial(self._set_gradient_checkpointing, value=True, n_blocks=n_first_temporal_block))
    ...
    def forward(
        ...
        if self.is_input_patches: # here
            ...
            # (B F) T D
            hidden_states = self.pos_embed(hidden_states) # patchify + spatial pos embed
        ...
        for i, (spatial_block, temp_block) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks)):
            ...
            if self.training and self.gradient_checkpointing and i <= self.n_gradient_checkpointing_blocks:
...
def FrameDiT_H_T2V_XL_2_Latte_like(**kwargs):
    return FrameDiT_H_T2V(
...
def FrameDiT_H_T2V_XL_2_1_2_concat(**kwargs):
    return FrameDiT_H_T2V_XL_2_Latte_like(num_col_attn_heads=512, qk_col_attn_head_dim=1, v_col_attn_head_dim=2, fuse_mode='concat', u_type='pure-softmax', attention_mode='math', use_bias=True, bias_type='col_row', **kwargs)



FrameDiTHT2V_models = {
    'FrameDiTHT2V-XL/256-2-1-2-concat': FrameDiT_H_T2V_XL_2_1_2_concat,
}
