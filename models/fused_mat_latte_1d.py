...
class FusedMatLatte1D(nn.Module):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.patch_size = 1
        self.n_tokens_per_frames = (input_size // self.patch_size) ** 2
        self.num_col_heads = num_col_heads
        self.num_row_heads = num_row_heads
        self.extras = extras
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_1d_sincos_temp_embed(self.pos_embed.shape[-1], int(self.x_embedder.num_patches))
    ...
    def forward(self, 
        ...
        x = rearrange(x, 'b (t l) c -> b t l c', t=frames)           
        #x = self.unpatchify(x)                  
...
#============================================================================================
def FusedMatLatte1D_XL(**kwargs):
    return FusedMatLatte1D(depth=28, hidden_size=1152, num_row_heads=16, **kwargs)

# def FusedMatLatte_XL_4(**kwargs):
#     return FusedMatLatte1D(depth=28, hidden_size=1152, patch_size=4, num_row_heads=16, **kwargs)

# def FusedMatLatte_XL_8(**kwargs):
#     return FusedMatLatte1D(depth=28, hidden_size=1152, patch_size=8, num_row_heads=16, **kwargs)

# For image size 256
def FusedMatLatte_XL_256_512_concat(**kwargs):
    return FusedMatLatte1D_XL(qk_col_dim=256, v_col_dim=512, num_col_heads=256, fuse_mode='concat', **kwargs)

def FusedMatLatte_XL_256_1024_concat(**kwargs):
    return FusedMatLatte1D_XL(qk_col_dim=256, v_col_dim=1024, num_col_heads=256, fuse_mode='concat', **kwargs)

def FusedMatLatte_XL_128_512_concat(**kwargs):
    return FusedMatLatte1D_XL(qk_col_dim=128, v_col_dim=512, num_col_heads=128, fuse_mode='concat', **kwargs)

def FusedMatLatte_XL_128_512_concat_softmax(**kwargs):
    return FusedMatLatte1D_XL(qk_col_dim=128, v_col_dim=512, num_col_heads=128, fuse_mode='concat', u_type='softmax', **kwargs)

def FusedMatLatte_XL_128_1024_concat(**kwargs):
    return FusedMatLatte1D_XL(qk_col_dim=128, v_col_dim=1024, num_col_heads=128, fuse_mode='concat', **kwargs)
...
#============================================================================================
FusedMatLatte1D_models = {
    'FusedMatLatte1D-XL/256-1024/2-concat': FusedMatLatte_XL_256_1024_concat,
}
...
if __name__ == '__main__':
    ...
    network = FusedMatLatte1D_XL().to(device)
