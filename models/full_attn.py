...
class DiT3D(nn.Module):
    ...
    def forward(self, 
        ...
        x = rearrange(x, '(b f) n d -> (b n) f d', b=batches, f=frames) + self.temp_embed

        t = self.t_embedder(t, use_fp16=use_fp16)
        timestep = repeat(t, 'b d -> b (f n) d', f=self.temp_embed.shape[1], n=self.pos_embed.shape[1])
        ...
        x = rearrange(x, 'b (f n) d -> (b f) n d', b=batches, f=frames)
        x = self.unpatchify(x)                  
