...
class SpatialDiffLatteV2(nn.Module):
    ...
    def forward(self, 
        ...
        x, dx = uncombine_tensors(x, combine_type='token_interleave') # B, F, C, H, W ; B, F, C, H, W
        ...
        # stack interleave
        x = combine_tensors(x, dx, combine_type='token_interleave') # B*F, N*2, D
        ...
        x, dx = uncombine_tensors(x, combine_type='token_interleave') # B*F, N, D ; B*F, N, D
        x = self.unpatchify(x)
        dx = self.unpatchify(dx)

        x = combine_tensors(x, dx, combine_type='token_interleave') # B, F, C, H, W
