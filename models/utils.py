...
def load_pretrained_latte_into_framedith(framedith_model, latte_pretrained_path, logger, device='cpu'):
    ...
    # for name, param in framedith_model.named_parameters():
    #     if "alpha" in name and "attn1" in name:
    #         print(f"Zero-initializing gate: {name}")
...
if __name__ == "__main__":
    # 1. Define Config matching your pretrained Latte
    # (Example config based on Latte-XL/2)
    col_dim = 256 # 512x512 / (16*16 patch) ? Or 256 tokens? Adjust to match Latte config
    from .framedit_h_t2v import FrameDiTHT2V
    ...
    # 2. Path to pretrained Latte (HuggingFace cache or local folder)
    # Ensure this path contains config.json and diffusion_pytorch_model.bin
    pretrained_path = "maxin-cn/Latte-1" 
