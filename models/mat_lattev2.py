...
class MatrixLinearV2(nn.Module):
    def __init__(
        self,
        ...
        self.u_projection = nn.Linear(in_features[0], out_features[0], bias=False, **factory_kwargs)
        self.v_projection = nn.Linear(in_features[1], out_features[1], bias=False, **factory_kwargs)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        input: (..., N, D)
        """
        x = self.v_projection(input)
        x = rearrange(x, '... n d -> ... d n')
        x = self.u_projection(x)
...
class MatrixAttention(nn.Module):
    ...
    def __init__(
        ...
        self.linear_q = MatrixLinearV2((self.col_dim, self.row_dim), (self.qk_col_dim, self.row_dim), bias=self.use_bias)
        self.linear_k = MatrixLinearV2((self.col_dim, self.row_dim), (self.qk_col_dim, self.row_dim), bias=self.use_bias)
        self.linear_v = MatrixLinearV2((self.col_dim, self.row_dim), (self.v_col_dim, self.row_dim), bias=self.use_bias)

        self.proj_v = MatrixLinearV2((self.v_col_dim, self.row_dim), (self.col_dim, self.row_dim), bias=self.use_bias)
...
#################################################################################
#                                 Core MatLatteV2 Model                                #
#################################################################################

class TransformerBlock(nn.Module):
    """
    A MatLatteV2 tansformer block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
...
class MatLatteV2(nn.Module):
    ...
    def initialize_weights(self):
        ...
        def _basic_init(module):
            ...
            elif isinstance(module, MatrixLinearV2):
        ...
        # Zero-out adaLN modulation layers in MatLatteV2 blocks:
    ...
    def forward_with_cfg(self, x, t, y=None, cfg_scale=7.0, use_fp16=False, text_embedding=None):
        """
        Forward pass of MatLatteV2, but also batches the unconditional forward pass for classifier-free guidance.
        """
...
#                                   MatLatteV2 Configs                                  #
#################################################################################
# def MatLatteV2_XL_64_128_2(**kwargs):
#     return MatLatteV2(depth=28, hidden_size=1152, patch_size=2, qk_col_dim=64, v_col_dim=128, num_col_heads=64, num_row_heads=16, **kwargs)

# def MatLatteV2_XL_64_128_2(**kwargs):
#     return MatLatteV2(depth=28, hidden_size=1152, patch_size=2, qk_col_dim=64, v_col_dim=128, num_col_heads=64, num_row_heads=16, **kwargs)

# def MatLatteV2_XL_256_256_2(**kwargs):
#     return MatLatteV2(depth=28, hidden_size=1152, patch_size=2, qk_col_dim=256, v_col_dim=256, num_col_heads=64, num_row_heads=16, **kwargs)
#
...
#============================================================================================
def MatLatteV2_XL_2(**kwargs):
    return MatLatteV2(depth=28, hidden_size=1152, patch_size=2, num_row_heads=16, **kwargs)

def MatLatteV2_XL_4(**kwargs):
    return MatLatteV2(depth=28, hidden_size=1152, patch_size=4, num_row_heads=16, **kwargs)

def MatLatteV2_XL_8(**kwargs):
    return MatLatteV2(depth=28, hidden_size=1152, patch_size=8, num_row_heads=16, **kwargs)

def MatLatteV2_L_2(**kwargs):
    return MatLatteV2(depth=24, hidden_size=1024, patch_size=2, num_row_heads=16, **kwargs)

def MatLatteV2_L_4(**kwargs):
    return MatLatteV2(depth=24, hidden_size=1024, patch_size=4, num_row_heads=16, **kwargs)

def MatLatteV2_L_8(**kwargs):
    return MatLatteV2(depth=24, hidden_size=1024, patch_size=8, num_row_heads=16, **kwargs)

def MatLatteV2_M_2(**kwargs):
    return MatLatteV2(depth=12, hidden_size=1024, patch_size=2, num_row_heads=16, **kwargs)

def MatLatteV2_M_4(**kwargs):
    return MatLatteV2(depth=12, hidden_size=1024, patch_size=4, num_row_heads=16, **kwargs)

def MatLatteV2_M_8(**kwargs):
    return MatLatteV2(depth=12, hidden_size=1024, patch_size=8, num_row_heads=16, **kwargs)

def MatLatteV2_B_2(**kwargs):
    return MatLatteV2(depth=12, hidden_size=768, patch_size=2, num_row_heads=12, **kwargs)

def MatLatteV2_B_4(**kwargs):
    return MatLatteV2(depth=12, hidden_size=768, patch_size=4, num_row_heads=12, **kwargs)

def MatLatteV2_B_8(**kwargs):
    return MatLatteV2(depth=12, hidden_size=768, patch_size=8, num_row_heads=12, **kwargs)

def MatLatteV2_S_2(**kwargs):
    return MatLatteV2(depth=12, hidden_size=384, patch_size=2, num_row_heads=6, **kwargs)

def MatLatteV2_S_4(**kwargs):
    return MatLatteV2(depth=12, hidden_size=384, patch_size=4, num_row_heads=6, **kwargs)

def MatLatteV2_S_8(**kwargs):
    return MatLatteV2(depth=12, hidden_size=384, patch_size=8, num_row_heads=6, **kwargs)

# For image size 256
def MatLatteV2_XL_64_512_2(**kwargs):
    return MatLatteV2_XL_2(qk_col_dim=64, v_col_dim=512, num_col_heads=64, **kwargs)

def MatLatteV2_XL_128_512_2(**kwargs):
    return MatLatteV2_XL_2(qk_col_dim=128, v_col_dim=512, num_col_heads=128, **kwargs)

def MatLatteV2_XL_128_1024_2(**kwargs):
    return MatLatteV2_XL_2(qk_col_dim=128, v_col_dim=1024, num_col_heads=128, **kwargs)

def MatLatteV2_XL_256_256_2(**kwargs):
    return MatLatteV2_XL_2(qk_col_dim=256, v_col_dim=256, num_col_heads=256, **kwargs)

def MatLatteV2_XL_256_512_2(**kwargs):
    return MatLatteV2_XL_2(qk_col_dim=256, v_col_dim=512, num_col_heads=256, **kwargs)

# For image size 128, 64 tokens per frame
def MatLatteV2_M_4_256_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=4, v_col_dim=256, num_col_heads=4, **kwargs)

def MatLatteV2_M_4_512_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=4, v_col_dim=512, num_col_heads=4, **kwargs)

def MatLatteV2_M_8_256_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=8, v_col_dim=256, num_col_heads=8, **kwargs)

def MatLatteV2_M_16_512_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=16, v_col_dim=512, num_col_heads=16, **kwargs)

def MatLatteV2_M_16_256_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=16, v_col_dim=256, num_col_heads=16, **kwargs)

def MatLatteV2_M_16_64_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=16, v_col_dim=64, num_col_heads=16, **kwargs)

def MatLatteV2_M_32_256_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=32, v_col_dim=256, num_col_heads=32, **kwargs)

def MatLatteV2_M_64_64_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=64, v_col_dim=64, num_col_heads=64, **kwargs)

def MatLatteV2_M_64_256_2(**kwargs):
    return MatLatteV2_M_2(qk_col_dim=64, v_col_dim=256, num_col_heads=64, **kwargs)

# For image size 64
def MatLatteV2_B_16_16_2(**kwargs):
    return MatLatteV2_B_2(qk_col_dim=16, v_col_dim=16, num_col_heads=16, **kwargs)
#============================================================================================


MatLatteV2_models = {
    
    'MatLatteV2-M/64-64/2': MatLatteV2_M_64_64_2, 'MatLatteV2-M/64-256/2': MatLatteV2_M_64_256_2, 
    'MatLatteV2-B/16-16/2': MatLatteV2_B_16_16_2, 'MatLatteV2-M/16-64/2': MatLatteV2_M_16_64_2, 'MatLatteV2-M/16-256/2': MatLatteV2_M_16_256_2, 'MatLatteV2-M/16-512/2': MatLatteV2_M_16_512_2,
    'MatLatteV2-M/32-256/2': MatLatteV2_M_32_256_2,
    'MatLatteV2-M/8-256/2': MatLatteV2_M_8_256_2, 
    'MatLatteV2-M/4-256/2': MatLatteV2_M_4_256_2, 'MatLatteV2-M/4-512/2': MatLatteV2_M_4_512_2, 
    'MatLatteV2-XL/64-512/2': MatLatteV2_XL_64_512_2, 'MatLatteV2-XL/128-512/2': MatLatteV2_XL_128_512_2, 'MatLatteV2-XL/256-256/2': MatLatteV2_XL_256_256_2,
    'MatLatteV2-XL/128-1024/2': MatLatteV2_XL_128_1024_2, 'MatLatteV2-XL/256-512/2': MatLatteV2_XL_256_512_2
}
...
if __name__ == '__main__':
    ...
    network = MatLatteV2_XL_2().to(device)
