...
class VideoCaptionDataset(Dataset):
    def __init__(self, csv_path, output_path, shard_size):
        super().__init__()
    ...
    def __getitem__(self, index):
        item = self.raw_data.iloc[index][['ID', 'Start Time', 'End Time', 'Brief Caption']]
        # to dict
        item = item.to_dict()
        return item
    ...
    def load_raw_data(self):
        ...
        df = df[~df["ID"].isin(existing_video_ids)]
        pattern = r"^\d{1,2}:[0-5]\d:[0-5]\d\.\d{3}$"
        mask = (
            df["Start Time"].str.fullmatch(pattern) &
            df["End Time"].str.fullmatch(pattern)
        )
        df = df[mask]
...
def read_video(path, start_time, end_time, n_frames=16, train_fps=8):
...
def main():
    ...
    parser.add_argument("--csv_path", type=str, required=True, help="Path to the csv file containing video metadata")
    ...
    dataset = VideoCaptionDataset(args.csv_path, args.output_path, args.shard_size)
    ...
    for i, batch in enumerate(dataloader):
        ...
        prompts = batch['Brief Caption'] # list of str

        # download video, decode frames, apply transforms, and move to device
        video_idxs = batch["ID"]
        video_names = [f"{video_idx}.mp4" for video_idx in video_idxs]
        local_video_paths = [os.path.join(args.video_path, video_id+'.mp4') for video_id in video_idxs]

        start_time = batch['Start Time'].tolist()
        end_time = batch['End Time'].tolist()
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            results = list(executor.map(read_video, local_video_paths, start_time, end_time, [args.n_frames]*len(local_video_paths), [args.train_fps]*len(local_video_paths)))
        ...
        filter_func = lambda list_values: [v for idx, v in enumerate(list_values) if idx not in skip_items]
        prompts = filter_func(prompts)
        video_idxs = filter_func(video_idxs)
        ...
        for j in range(latent.size(0)):
            ...
            shard_idx = random.randint(0,args.shard_size) 
            shard_dir = opj(args.output_path, f"{shard_idx:04d}")
            os.makedirs(shard_dir, exist_ok=True)
            output_file = opj(shard_dir, f"{video_id}.pt")
...
"""
    parser = argparse.ArgumentParser(description="Process raw videos into latents for training")
    parser.add_argument("--csv_path", type=str, required=True, help="Path to the parquet file containing video metadata")
    parser.add_argument("--video_path", type=str, required=True, help="Path to the directory containing video files")
    parser.add_argument("--output_path", type=str, required=True, help="Path to save the processed latents")
    parser.add_argument("--model_path", type=str, default="maxin-cn/Latte-1", help="Path to the pretrained model for VAE")
    parser.add_argument("--num_workers", type=int, default=4, help="Number of workers for data loading")
...
"""
srun python scripts/videoufo/ddp_process_latents_v1.py \
    --csv_path /weka/s224075134/temporal_diffusion/datasets/video/videoufo/VideoUFO.csv \
    --model_path maxin-cn/Latte-1 \
    --output_path /weka/s224075134/temporal_diffusion/datasets/video/videoufo/autoencoder_kl_latents \
    --video_path /weka/s224075134/temporal_diffusion/datasets/video/videoufo/raw/VideoUFO_tar/VideoUFO \
    --num_workers 4
"""