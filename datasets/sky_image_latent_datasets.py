...
class SkyImagesLatent(data.Dataset):
    def __init__(self, configs, transform, temporal_sample=None, train=True):
        ...
        self.data_all, self.video_frame_all = self.load_video_frames_latent(self.data_path)
    ...
    def __getitem__(self, index):
        ...
        data = self.data_all[video_index]
        if isinstance(data, list):
            # randomly select one chunk from the list
            data_idx = random.randint(0, len(data)-1)
            data = data[data_idx]
        video_path, total_frames = data
    ...
    def load_video_frames_latent(self, dataroot):
        ...
        latent_files = []
        latent_files = os.listdir(dataroot)
        latent_files = sorted(latent_files, key=self.chunk_sort_key)

        chunk_groups = {}
        for file in latent_files:
            if file.lower().endswith('.pt'):
                latent = torch.load(os.path.join(dataroot, file), weights_only=False)
                n_frames = latent.shape[0]
                # print('file', file, n_frames)

                if '_chunk' in file:
                    # Group by base name and flip
                    m = re.match(r'(.+)_chunk_(\d+)(_flip)?\.pt$', file)
                    if m:
                        base = m.group(1)
                        is_flip = m.group(3) is not None
                        group_key = f"{base}_flip" if is_flip else base
                        if group_key not in chunk_groups:
                            chunk_groups[group_key] = []
                        chunk_groups[group_key].append(file)
                else:
                    if n_frames > 0:
                        data_all.append((os.path.join(dataroot, file), n_frames))

                if n_frames > 0:
                    for i in range(n_frames):
                        frames_all.append((os.path.join(dataroot, file), i))

        # Now sort each chunk group and append as a list
        for group in chunk_groups.values():
            # Sort by chunk number
            group_sorted = sorted(group, key=self.chunk_sort_key)
            group_files = []
            for f in group_sorted:
                latent = torch.load(os.path.join(dataroot, f), weights_only=False)
                n_frames = latent.shape[0]
                if n_frames > 0:
                    group_files.append((os.path.join(dataroot, f), n_frames))
            data_all.append(group_files)

        return data_all, frames_all
    
    def chunk_sort_key(self, filename):
        # Match chunk number and flip
        if '_chunk' in filename:
            m = re.search(r'_chunk_(\d+)(?:_flip)?\.pt$', filename)
            if m:
                chunk_num = int(m.group(1))
                is_flip = '_flip' in filename
                # Files with chunk: sort by chunk_num, non-flip before flip, after non-chunk files
                return (1, is_flip, chunk_num)
            else:
                # Fallback for chunk files not matching pattern
                return (1, float('inf'), True)
        else:
            # Non-chunk files come first, keep their original order
            return (0, 0, False)
