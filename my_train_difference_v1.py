...
def main(args):
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(loader):
            ...
            xs = video_data['video'].to(device, non_blocking=True)
            video_name = video_data['video_name']
            
            if not args.load_latent:
                xs = encode_video(vae, xs)  # (B,F,C,H,W)
            xs = xs.mul_(vae.scaler)

            # Get difference
            #TODO: add diff norm
            diff = get_difference(xs)
            ...
            loss_dict = diffusion.diff_training_losses(model, x_start=xs, diff_start=diff, t=t, model_kwargs=model_kwargs)
            ...
            # for logging
            running_metrics['loss'] += loss_dict['loss'].mean().item() / args.gradient_accumulation_steps
            running_metrics['xs_loss'] += loss_dict['x_loss'].mean().item() / args.gradient_accumulation_steps if 'x_loss' in loss_dict else 0.0
            running_metrics['diff_loss'] += loss_dict['diff_loss'].mean().item() / args.gradient_accumulation_steps if 'diff_loss' in loss_dict else 0.0
            ...
            running_metrics['diff_mse'] += loss_dict['diff_mse'].mean().item() / args.gradient_accumulation_steps if 'diff_mse' in loss_dict else 0.0
            ...
            running_metrics['diff_vb'] += loss_dict['diff_vb'].mean().item() / args.gradient_accumulation_steps if 'diff_vb' in loss_dict else 0
            ...
            if train_steps % args.log_every == 0 or train_steps == 1:
                ...
                logger.info(f"(step={train_steps:07d}/epoch={epoch:04d}) Loss: {reduced_metrics['loss']:.4f}, xs_loss: {reduced_metrics['xs_loss']:.4f}, diff_loss: {reduced_metrics['diff_loss']:.4f}, grad_norm: {gradient_norm:.4f}, Train Steps/Sec: {steps_per_sec:.2f}")
