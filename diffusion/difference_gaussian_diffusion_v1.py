...
from .gaussian_diffusion import mean_flat, _extract_into_tensor, ModelMeanType, ModelVarType, LossType
...
class DifferenceGaussianDiffusion_v1:
    ...
    def diff_training_losses(self, model, x_start, diff_start, t, model_kwargs=None, x_noise=None):
        """
        Compute training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param z_start: the [N x C x ...] tensor of inputs.
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if x_noise is None:
            x_noise = th.randn_like(x_start)
        diff_noise = get_difference(x_noise) / np.sqrt(2)
        z_noise = combine_frames_and_difference(x_noise, diff_noise)

        z_start = combine_frames_and_difference(x_start, diff_start)

        x_t = self.q_sample(x_start, t, noise=x_noise)
        diff_t = self.q_sample(diff_start, t, noise=diff_noise)
        z_t = self.q_sample(z_start, t, noise=z_noise)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            z_model_output = model(z_t, t, **model_kwargs)
            ...
            if self.model_var_type in [
                ...
                B, F, C = z_t.shape[:3]
                assert z_model_output.shape == (B, F, C * 2, *z_t.shape[3:])
                z_model_output, z_model_var_values = th.split(z_model_output, C, dim=2)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = th.cat([z_model_output.detach(), z_model_var_values], dim=2)
                ...
                z_vb_no_mean = vb_term_outputs["output_no_mean"]
                x_vb_no_mean, diff_vb_no_mean = uncombine_frames_and_difference(z_vb_no_mean)
                terms['vb'] = mean_flat(z_vb_no_mean)
                terms['x_vb'] = mean_flat(x_vb_no_mean)
                terms['diff_vb'] = mean_flat(diff_vb_no_mean)
                terms['vb_no_mean'] = z_vb_no_mean
                terms['x_vb_no_mean'] = x_vb_no_mean
                terms['diff_vb_no_mean'] = diff_vb_no_mean

                if self.loss_type == LossType.RESCALED_MSE:
                    # Divide by 1000 for equivalence with initial implementation.
                    # Without a factor of 1/1000, the VB term hurts the MSE term.
                    terms["vb"] *= self.num_timesteps / 1000.0
                    terms["x_vb"] *= self.num_timesteps / 1000.0
                    terms["diff_vb"] *= self.num_timesteps / 1000.0
                    terms["vb_no_mean"] *= self.num_timesteps / 1000.0
                    terms["x_vb_no_mean"] *= self.num_timesteps / 1000.0
                    terms["diff_vb_no_mean"] *= self.num_timesteps / 1000.0

            z_target = {
                ModelMeanType.PREVIOUS_X:   self.q_posterior_mean_variance(x_start=z_start, x_t=z_t, t=t)[0],
                ModelMeanType.START_X:      z_start,
                ModelMeanType.EPSILON:      z_noise,
            ...
            diff_target = {
                ModelMeanType.PREVIOUS_X:   self.q_posterior_mean_variance(x_start=diff_start, x_t=diff_t, t=t)[0],
                ModelMeanType.START_X:      diff_start,
                ModelMeanType.EPSILON:      diff_noise,
            }[self.model_mean_type]
            x_model_output, diff_model_output = uncombine_frames_and_difference(z_model_output)

            assert z_model_output.shape == z_target.shape == z_start.shape
            assert x_model_output.shape == x_target.shape == x_start.shape
            assert diff_model_output.shape == diff_target.shape == diff_start.shape
            
            terms['mse_no_mean'] = (z_target - z_model_output) ** 2
            terms['x_mse_no_mean'] = (x_target - x_model_output) ** 2
            terms['diff_mse_no_mean'] = (diff_target - diff_model_output) ** 2
            terms["mse"] = mean_flat(terms['mse_no_mean'])
            terms["x_mse"] = mean_flat(terms['x_mse_no_mean'])
            terms["diff_mse"] = mean_flat(terms['diff_mse_no_mean'])

            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"]
                terms["x_loss"] = terms["x_mse"] + terms["x_vb"]
                terms["diff_loss"] = terms["diff_mse"] + terms["diff_vb"]
                terms["loss_no_mean"] = terms["mse_no_mean"] + terms["vb_no_mean"]
                terms["x_loss_no_mean"] = terms["x_mse_no_mean"] + terms["x_vb_no_mean"]
                terms["diff_loss_no_mean"] = terms["diff_mse_no_mean"] + terms["diff_vb_no_mean"]
            else:
                terms["loss"] = terms["mse"]
                terms["x_loss"] = terms["x_mse"]
                terms["diff_loss"] = terms["diff_mse"]
                terms["loss_no_mean"] = terms["mse_no_mean"]
                terms["x_loss_no_mean"] = terms["x_mse_no_mean"]
                terms["diff_loss_no_mean"] = terms["diff_mse_no_mean"]
        else:
