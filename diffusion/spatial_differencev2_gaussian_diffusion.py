...
from .gaussian_diffusion import _extract_into_tensor, mean_flat, ModelMeanType, ModelVarType, LossType
...
class SpatialDifferenceGaussianDiffusionV2:
    ...
    def p_sample(
        ...
        cond_fn=None,
        model_kwargs=None,
        tweedie_difference_threshold=None
    ):
        """
    ...
    def ddim_sample(
        ...
        model_kwargs=None,
        eta=0.0,
        tweedie_difference_threshold=0.0
    ):
        """
    ...
    def _vb_terms_bpd(
        ...
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs, tweedie_difference_threshold=None
        )
        kl = normal_kl(
    ...
    def training_losses(self, model, x_start, dx_start, t, model_kwargs=None, x_noise=None, dx_noise=None):
        ...
        x_start = combine_tensors(x_start, dx_start, combine_type='token_interleave')
        x_noise = combine_tensors(x_noise, dx_noise, combine_type='token_interleave')
        x_t = combine_tensors(x_t, dx_t, combine_type='token_interleave')
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                x_vb_no_mean, dx_vb_no_mean = uncombine_tensors(vb_no_mean, combine_type='token_interleave')
                terms["vb"] = vb
            ...
            x_mse_no_mean, dx_mse_no_mean = uncombine_tensors(mse_no_mean, combine_type='token_interleave')
