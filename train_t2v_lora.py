...
from utils import (clip_grad_norm_, create_logger, 
                   requires_grad, cleanup, setup_distributed,
                   get_experiment_dir, text_preprocessing, get_torch_dtype)
import numpy as np
...
def lora_stats(model, logger, key, tag=""):
    import torch
    with torch.no_grad():
        vals = []
        for n, p in model.named_parameters():
            if key in n:
                vals.append((n, p.abs().max().item(), p.norm().item(), p.abs().mean().item(), p.abs().std().item()))
        #vals = sorted(vals, key=lambda x: x[2], reverse=True)[:5]
        vals = vals[:5]

    logger.info(f"[{tag}] top {key} params: " + ", ".join([f"\n\t{n} max={max_val:.9f} norm={norm:.9f} mean={mean:.9f} std={std:.9f}" for n, max_val, norm, mean, std in vals]))
...
def main(args):
    ...
    if hasattr(args, 'use_lora') and args.use_lora:
        ...
        raise NotImplementedError("Currently only LoRA finetuning is supported.")

    # for name, param in base_model.named_parameters():
    #     if 'lora_' in name:
    #         logger.info(f"{name} - {param.abs().mean()} - {param.abs().std()}")
    ...
    if args.pretrained:
        checkpoint = torch.load(args.pretrained, map_location=lambda storage, loc: storage)['model']
    ...
    opt = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=args.learning_rate, weight_decay=0)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(loader):
            ...
            if train_steps == 0:
                lora_stats(model, logger, "lora_A", "before")
                lora_stats(model, logger, "lora_B", "before")
            ...
            if train_steps % 100 == 0:
                lora_stats(model, logger,"lora_A", "after")
                lora_stats(model, logger,"lora_B", "after")
