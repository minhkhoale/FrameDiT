...
from models.model_utils import load_pretrained_latte_into_framedith, freeze_framedit_h_for_training
...
def main(args):
    ...
    base_model = get_models(deepcopy(args))
    ...
    logger.info('Load Latte checkpoint into FrameDiT-H-T2V')
    load_pretrained_latte_into_framedith(base_model, args.pretrained_latte, logger, device)

    if args.pretrained:
        logger.info('Load pretrained weights from {}'.format(args.pretrained))
    ...
    # Freeze model parameters for matrix training
    freeze_framedit_h_for_training(base_model, logger)
        ...
        for step, video_data in enumerate(loader):
            ...
            with accelerator.accumulate(model):
                ...
                if accelerator.sync_gradients:
                    grad_norm = accelerator.clip_grad_norm_(model.parameters(), float('inf') if train_steps < args.start_clip_iter else args.clip_max_norm)  # just compute grad norm without clipping
